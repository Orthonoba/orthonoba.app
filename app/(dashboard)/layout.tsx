...
import {
  LayoutDashboard, Users, Briefcase, ClipboardList, Stethoscope,
  Building2, FlaskConical, GraduationCap, Zap, TrendingUp,
  Brain, CreditCard, ShieldCheck, Settings, Bell, Search,
  ChevronDown, ChevronRight, Menu, X, LogOut, Box,
  Truck, FileText, BarChart2, Megaphone, Hash,
  BookOpen, PlayCircle, UserCheck, Key, User,
  DollarSign, Receipt, Tag, Star, Cpu,
  Scan, Library, Layers, Workflow, Bot, Wrench,
  Mail, MessageCircle, UsersRound, Calendar, Kanban,
  PieChart, Map, Clock,
} from 'lucide-react'
...
const NAV: NavGroup[] = [
    ...
    items: [
      {
        ...
        icon: UsersRound,
        children: [
          { href: '/crm',              label: 'Resumen CRM',    icon: UsersRound },
          { href: '/crm/pipeline',     label: 'Pipeline',       icon: Kanban },
          { href: '/crm/leads',        label: 'Leads',          icon: TrendingUp },
          { href: '/crm/patients',     label: 'Pacientes CRM',  icon: Users },
          { href: '/crm/appointments', label: 'Citas',          icon: Calendar },
        ],
      },
      {
        href: '/patients',
        ...
        icon: Users,
        children: [
          { href: '/patients',     label: 'Todos',          icon: Users },
    ...
    items: [
      {
        ...
        children: [
          ...
          { href: '/cases/files',    label: 'Archivos',        icon: FileText },
        ],
      },
      {
        href: '/orders',
        ...
        badge: '5',
        children: [
          { href: '/orders',         label: 'Todas',           icon: ClipboardList },
          { href: '/orders/new',     label: 'Nueva orden',     icon: FileText },
          { href: '/orders/pickup',  label: 'Recogida',        icon: Truck, badge: 'Nuevo' },
        ],
      },
    ],
  },
    ...
    items: [
      {
        ...
        icon: Scan,
        children: [
          { href: '/stl-upload',   label: 'Subir STL',      icon: Scan },
          { href: '/stl-library',  label: 'Biblioteca',     icon: Library },
          { href: '/files/stl',    label: 'Gestión 3D',     icon: Box },
        ],
      },
      {
        href: '/cad-design',
        ...
        icon: Layers,
        children: [
          { href: '/cad-design',         label: 'Panel CAD',    icon: Layers },
    ...
    items: [
      {
        ...
        badge: 'Beta',
        children: [
          { href: '/ai-automation',            label: 'Centro IA',    icon: Brain },
          { href: '/ai-automation/workflows',  label: 'Workflows',    icon: Workflow },
          { href: '/ai-automation/n8n',        label: 'N8N',          icon: Zap },
          { href: '/ai-automation/agents',     label: 'Agentes',      icon: Bot },
        ],
      },
      {
        href: '/automation',
    ...
    items: [
      {
        ...
        icon: Megaphone,
        children: [
          { href: '/marketing/leads',     label: 'Leads & CRM',     icon: TrendingUp },
          ...
          { href: '/marketing/funnels',   label: 'Funnels',          icon: BarChart2 },
    ...
    items: [
      {
        ...
        children: [
          ...
          { href: '/billing/plans',    label: 'Planes',               icon: Tag },
        ],
      },
    ],
  },
  {
    label: 'Plataforma',
    items: [
      { href: '/executive', label: 'Ejecutivo', icon: PieChart },
      {
        href: '/admin/users',
        label: 'Admin',
        icon: ShieldCheck,
      ...
      },
      {
        ...
        children: [
          ...
          { href: '/settings/team',      label: 'Equipo',    icon: UsersRound },
...
function SidebarContent({ onClose }: { onClose?: () => void }) {
  ...
  const BADGE_STYLE: Record<string, string> = {
    ...
    STL:   'bg-teal-500/20 text-teal-400',
  }

  function badgeClass(badge: string) {
    return BADGE_STYLE[badge] ?? 'bg-sky-500/20 text-sky-400'
  }
    ...
    <div className="flex flex-col h-full bg-slate-900 border-r border-slate-700/60 overflow-hidden">
                  ...
                  {item.children ? (
                    <button
                      ...
                      className={[
                        ...
                        parentActive ? 'text-sky-400 bg-sky-500/10' : 'text-slate-400 hover:text-slate-200 hover:bg-slate-800',
                      ].join(' ')}
                    >
                      <item.icon className="w-4 h-4 shrink-0" />
                      <span className="flex-1 text-left">{item.label}</span>
                      {item.badge && <span className={`text-[10px] font-bold px-1.5 py-0.5 rounded-full shrink-0 ${badgeClass(item.badge)}`}>{item.badge}</span>}
                    ...
                    <Link href={item.href} onClick={onClose}
                      className={[
                        ...
                        isActive(item.href) ? 'text-sky-400 bg-sky-500/10 border border-sky-500/20' : 'text-slate-400 hover:text-slate-200 hover:bg-slate-800',
                      ].join(' ')}
                    >
                      <item.icon className="w-4 h-4 shrink-0" />
                      ...
                      {item.children.map((child) => (
                        <Link key={child.href} href={child.href} onClick={onClose}
                          className={[
                            ...
                            isActive(child.href) ? 'text-sky-400 bg-sky-500/10' : 'text-slate-500 hover:text-slate-300 hover:bg-slate-800',
                          ].join(' ')}
                        >
                          {child.icon && <child.icon className="w-3.5 h-3.5 shrink-0" />}
                          <span>{child.label}</span>
                          {child.badge && <span className={`ml-auto text-[10px] font-bold px-1.5 py-0.5 rounded-full ${badgeClass(child.badge)}`}>{child.badge}</span>}
                        </Link>
                      ))}
                    </div>
                  )}
      ...
      <div className="px-3 py-3 border-t border-slate-700/60 shrink-0">
        <div className="flex items-center gap-2.5 px-3 py-2 rounded-xl hover:bg-slate-800 cursor-pointer group transition">
          <div className="w-8 h-8 rounded-full bg-linear-to-br from-sky-500 to-violet-500 flex items-center justify-center text-white text-xs font-bold shrink-0">JG</div>
          <div className="flex-1 min-w-0">
    ...
    <div className="min-h-screen bg-slate-900 flex">
      ...
      <div className="flex-1 flex flex-col min-w-0 lg:pl-[260px]">
        <header className="sticky top-0 z-40 flex items-center gap-3 h-14 px-4 bg-slate-900/95 backdrop-blur border-b border-slate-700/60 shrink-0">
          <button onClick={() => setMobileOpen(true)} className="lg:hidden text-slate-400 hover:text-white p-1.5 rounded-lg hover:bg-slate-800 transition">
            ...
            {breadcrumbs.map((crumb, i) => (
              <span key={crumb.href} className="flex items-center gap-1">
                ...
                {i === breadcrumbs.length - 1 ? (
                  ...
                  <Link href={crumb.href} className="text-slate-500 hover:text-slate-300 transition">{crumb.label}</Link>
                )}
              </span>
            ))}
          </nav>
          <div className="flex-1" />
          <button className="flex items-center gap-2 px-3 py-1.5 rounded-lg bg-slate-800 border border-slate-700 text-slate-400 text-sm hover:border-sky-500/50 transition">
          ...
          <div className="w-8 h-8 rounded-full bg-linear-to-br from-sky-500 to-violet-500 flex items-center justify-center text-white text-xs font-bold cursor-pointer ring-2 ring-transparent hover:ring-sky-500/50 transition">JG</div>
        </header>
